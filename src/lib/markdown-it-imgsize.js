function parseImageSize(str, pos, max) {
    let src = str.substring(pos);
    let match = src.match(/(.*\s*\[(\d*\.?\d*(?:px|em|rem|%)?)?\s*(?:[xX]\s*(\d*\.?\d*(?:px|em|rem|%)?)?)?\])/);
    if (match) {
        return {
            ok: true,
            width: match[2] || '',
            height: match[3] || '',
            pos: pos + match[1].length
        };
    }
    return { ok: false, width: '', height: '', pos: 0 };
}

export default function markdownItImageSize(md) {
    md.inline.ruler.before('emphasis', 'image', function (state, silent) {
        var attrs,
            code,
            label,
            labelEnd,
